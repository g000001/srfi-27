...
(define-function (check-mrg32k3a)
  ...
  (let ((s (make-random-source)))
    ...
    (if (not (equalp (random-source-state-ref s)
		     '(lecuyer-mrg32k3a
		       1250826159
		       3004357423
		        431373563
  ...
  (let* ((x 0.0)
    ...
    (do ((k 0 (+ k 1)))
	((= k 10000000)
	 (if (not (< (abs (- x 5001090.95d0)) 0.01d0))
	     (error "bad sum over 10^7 reals" x)))
      (set! x (+ x (funcall rand)))))
  (display "ok")
  (newline)
  'T)
...
(define-function (write-diehard filename s bytes-per-call calls)
  (with-open-file (port filename
    ...
    (let ((rand (random-source-make-integers s))
      ...
      (do ((i 0 (+ i 1)))
          ((= i calls))
        (let ((x (funcall rand n)))
          (do ((k 0 (+ k 1))) ((= k bytes-per-call))
            (write-char (code-char (modulo x 256)) port)
            (set! x (quotient x 256))))))))
...
(test check
  (is-true (check-basics-1))
  (display "passed (check-basics-1)")
  (newline)
  (is-true (check-mrg32k3a))
  (display "passed (check-basics-1)")
  (newline))
