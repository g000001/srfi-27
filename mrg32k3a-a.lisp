...
(define-function (mrg32k3a-random-m1 state)
...
(declaim (inline mrg32k3a-pack-state))
(define-function (mrg32k3a-pack-state unpacked-state)
  (declare (optimize (safety 0) (speed 3) (debug 0)))
  unpacked-state)

(declaim (inline mrg32k3a-unpack-state))
(define-function (mrg32k3a-unpack-state state)
  (declare (optimize (safety 0) (speed 3) (debug 0)))
  state)

(declaim (inline mrg32k3a-random-range))
(define-function (mrg32k3a-random-range) ; m1
  (declare (optimize (safety 0) (speed 3) (debug 0)))
  4294967087)

(define-function (mrg32k3a-random-integer state range) ; rejection method
  (let* ((q (quotient 4294967087 range))
	 (qn (* q range)))
    (do ((x (mrg32k3a-random-m1 state) (mrg32k3a-random-m1 state)))
	((< x qn) (quotient x q)))))

(define-function (mrg32k3a-random-real state) ; normalization is 1/(m1+1)
  (* 0.0000000002328306549295728d0 (+ 1.0d0 (mrg32k3a-random-m1 state))))
